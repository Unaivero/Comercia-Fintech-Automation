...
public class PaymentAPISteps {
    ...
    // private String baseUri = "https://api.example-fintech.com/payments"; // Mock API base URI - Now read from config
    ...
    public void a_payment_transaction_with_id_has_been_processed(String txnId) {
        ...
        // In a real scenario, this might involve setting up test data
        // or ensuring a transaction from a previous step (e.g., UI payment) exists.
        // For this mock, we just store the ID.
    }

    @When("I send a GET request to the {string} endpoint")
    public void i_send_a_get_request_to_the_endpoint(String endpointPath) {
        String apiBaseUri = ConfigReader.getProperty("api.base.uri");
        ...
        System.out.println("Sending GET request to: " + apiBaseUri + actualEndpoint);
        ...
        // For now, we simulate a response based on the transactionId.
        if (this.transactionId.endsWith("success")) {
            response = RestAssured.given()
                .when()
                .get("https://run.mocky.io/v3/0199c398-9657-4578-9e07-f609093f0b0e"); // Mocky returning 200 and status: Captured
            System.out.println("Mocked successful API response.");
        } else {
            response = RestAssured.given()
                .when()
                .get("https://run.mocky.io/v3/42042250-0126-4634-839c-90d512e62a0c"); // Mocky returning 404 or other status
            System.out.println("Mocked other API response.");
        }
    }

    @Then("the API response status code should be {int}")
    public void the_api_response_status_code_should_be(Integer expectedStatusCode) {
        System.out.println("Verifying API response status code. Expected: " + expectedStatusCode + ", Actual: " + response.getStatusCode());
    ...
    public void the_api_response_should_indicate_the_payment_status_is(String status1, String status2) {
        ...
        assertTrue(status1.equalsIgnoreCase(paymentStatus) || status2.equalsIgnoreCase(paymentStatus));
        System.out.println("Payment status assertion passed.");
    }

    // --- Mock API Endpoints (Conceptual - actual mocking done via Mocky for simplicity) ---
    // In a more complex setup, you might use WireMock or a similar library to run a local mock server.

    // Example structure for /authorize (not used in current features but for completeness)
    public Response authorizePayment(String cardNumber, String expiry, String cvv, double amount) {
        ...
        // return request.post("/authorize"); // Real call
        System.out.println("Mocking POST /authorize");
        return RestAssured.given().when().get("https://run.mocky.io/v3/0199c398-9657-4578-9e07-f609093f0b0e"); // Dummy response
    }

    // Example structure for /capture (not used in current features)
    public Response capturePayment(String transactionId, double amount) {
        ...
        // return request.post("/capture"); // Real call
